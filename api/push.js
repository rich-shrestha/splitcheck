...
export default async function handler(req, res) {
  ...
  const { expenses, twilioSid, twilioToken, twilioFrom, twilioTo } = req.body;
  if (!expenses?.length) return res.status(400).json({ error: 'No expenses' });

  const sessionId = crypto.randomBytes(6).toString('hex');
  const appUrl = `${process.env.APP_URL}/view/${sessionId}`;
  await redis.set(`session:${sessionId}`, JSON.stringify({ expenses, createdAt: Date.now() }), { ex: 43200 });
  ...
  if (twilioSid && twilioToken && twilioFrom && twilioTo) {
    ...
    try {
      ...
      smsSent=r.ok; if(!r.ok){const e=await r.json();smsError=e.message;}
